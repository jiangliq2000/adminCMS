      ...
      <el-dialog title="新增会员" :visible.sync ="addFormVisible" :close-on-click-modal="false">
        <el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
          ...
          <el-row type="flex" class="row-bg" justify="center">
            <el-button type="primary" @click="addStudentFormVisible = true" >添加学生</el-button>
          </el-row>
        </el-form>
        <div slot="footer" class="dialog-footer">
        ...
        <!--新增界面-->
        <el-dialog title="新增学生" :visible.sync ="addStudentFormVisible" :close-on-click-modal="false" append-to-body>
          ...
          <div slot="footer" class="dialog-footer">
            <el-button @click.native="addStudentFormVisible = false">取消</el-button>
  ...
  export default{
    data(){
      return {
        ...
        addStudentFormVisible: false,//新增界面是否显示
        addLoading: false,
        addFormRules: {
          name: [
            {required: true, message: '请输入会员姓名', trigger: 'blur'}
    ...
    methods: {
      ...
      addStudent: function () {
        ...
        this.$refs.addStudentForm.validate((valid) => {
          if (valid) {
            that.loading = true;
            let para = Object.assign({}, this.addStudentForm);
            para.createtime = new Date().toLocaleDateString();
            para.changetime = new Date().toLocaleDateString();
            console.log("create time is ");
            console.log(para.createtime);
            API_STUDENT.add(para).then(function (result) {
              that.loading = false;
              if (result && parseInt(result.errcode) === 0) {
                that.$message.success({showClose: true, message: '创建新会员成功', duration: 2000});
                that.$refs['addStudentForm'].resetFields();
