  ...
  <el-row class="warp">
    ...
    <el-col :span="24" class="warp-main" v-loading="loading" element-loading-text="拼命加载中">
      ...
      <el-table :data="members" highlight-current-row @selection-change="selsChange"
        ...
        <el-table-column prop="memtype" label="会员级别" :formatter="formatMemtype" sortable></el-table-column>
        <el-table-column prop="mobile" label="手机"  width="110"></el-table-column>
        <el-table-column prop="othermobiles" label="关联手机"  width="110"></el-table-column>        
      ...
      <el-dialog title="关联学生" :visible.sync ="corStdFormVisible" :close-on-click-modal="false">
        <el-form :model="corStdeditForm" label-width="100px"  ref="corStdeditForm">
          ...
          <el-row>
            <el-col :span="8">
              ...
              <el-select v-model="corStdoption" clearable placeholder="请选择待关联学生" no-data-text="请先查询学生">
                <!--
                <el-option
            ...
            <el-col :span="8">
              <el-form-item>
                <el-button type="primary" v-on:click="corStdAdd">添加</el-button>
              </el-form-item>
            </el-col>            
            <!--
            <el-col :span="8">
        ...
        <el-table :data="corStdeditForm.GridData">
          <el-table-column property="name" label="学生姓名" ></el-table-column>
          <el-table-column property="urgent_mobile" label="联系人手机" ></el-table-column>
          <el-table-column property="relationship" :formatter="formatRelationship" label="关系"></el-table-column>
          <!--
          <el-table-column property="focus" label="关注度"></el-table-column>
          -->
          <el-table-column label="操作" width="80">
             <template slot-scope="scope1">
               <el-button size="small" @click="DelStdGuard(scope1.$index,scope1.row)">删除</el-button>
             </template>
          </el-table-column>
        </el-table>         
...
-->
      ...
      <el-dialog title="编辑" :visible.sync ="editFormVisible" :close-on-click-modal="false">
        <el-form :model="editForm" label-width="100px" :rules="editFormRules" ref="editForm">
          ...
          <el-form-item label="关联手机" prop="othermobiles" auto-complete="off" :disabled="true">
            <el-input v-model="editForm.othermobiles" auto-complete="off"></el-input>
          </el-form-item>
      ...
      <el-tabs v-model="editableTabsValue" type="card" :before-leave="beforeLevaeTab" editable @edit="handleTabsEdit" @tab-click="clickTab">
        <el-tab-pane label="会员信息" name="member">
          ...
          <el-form-item label="关联手机" prop="othermobiles" >
            <el-input v-model="addForm.othermobiles" auto-complete="off" placeholder="手机号请以分号隔开"></el-input>
          </el-form-item>    
          <el-form-item label="学生关系">
  ...
  export default{
    ...
    methods: {
      formatRelationship(row, column){
        return row.relationship == 1 ? "母亲" : row.relationship == 2 ? "父亲" : row.relationship == 3 ? "爷爷" : row.relationship == 4 ? "奶奶" : row.relationship == 5 ? "外公" : row.relationship == 6 ? "外婆" :  "其他";
      },
      formatSex(row, column){
        return row.sex == 1 ? "男" : "女";
      },      
      formatMemtype(row, column){
        return row.memtype == 0 ? "非付费会员" : "普通会员";
      },      
      ...
      corStdAdd(){
        let param = this.corStdoption;
        let that = this;

        APISTD.findBySid(param).then(function (result) {
          console.log(result);
          if (result && result.stdinfos) {
            ...
            var isKeyExist = 0;
            for(var j=0;j<that.corStdeditForm.GridData.length;j++){
              if (that.corStdoption == that.corStdeditForm.GridData[j]['id']){
                isKeyExist = 1;
                break;
              }
            }
            if (isKeyExist == 0){
              that.corStdeditForm.GridData.push({'id':that.corStdoption,'name': result.stdinfos['name'] ,'urgent_mobile':result.stdinfos['urgent_mobile'] ,'relationship':that.corStdeditForm.relation});
            }
      ...
      corStdeditSubmit: function () {
        let that = this;
        that.loading = true;
        let para = Object.assign({}, that.corStdeditForm.GridData);
        console.log("corStdeditSubmit");
        console.log(para);

        let params = {
          corStudents: that.corStdeditForm.GridData
        };
        ...
        that.corStdOptions = [] ;
        that.corStdoption = "";
        console.log(that.corStdOptions);
        console.log(that.corStdoption);
      },



      // 删除关联学生
      DelStdGuard: function (index, row) {
        console.log("showStdGuardEditDialog");
        console.log(index);
        console.log(row);
        console.log(row.id);
        console.log(this.corStdeditForm.GridData);
        let indexflag = -1;
        for(var i=0; i<this.corStdeditForm.GridData.length; i++){
          //console.log(this.corStdeditForm.GridData[i].id);
          if (this.corStdeditForm.GridData[i].id == row.id){
            indexflag = i;               
          }
        }
        console.log(indexflag);
        this.corStdeditForm.GridData.splice(indexflag,1);
      },
      handleSearch(){
        this.total = 0;
        this.page = 1;
      ...
      showStdDialog: function (index, row) {
        ...
        APISTDGARD.findByMemberId(that.corStdeditForm.memid).then(function (result) {
              console.log(result);
              if (result) {

                console.log(" corelation student is ");
                console.log(result.students);
                that.corStdeditForm.GridData = result.students;

              } else {
                that.$message.error({showClose: true, message: '获取关联学生失败', duration: 2000});
              }
        }, function (err) {
          that.loading = false;
          that.$message.error({showClose: true, message: err.toString(), duration: 2000});
